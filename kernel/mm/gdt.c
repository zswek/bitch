...
void gdt_init(void) {
    ...
    set_segment_descriptor(&gdt_list[GDT_TSS_INDEX], (_size_t) &globl_tss + PAGE_OFFSET, sizeof(_task_state_segment_t),
    DESC_P|DESC_DPL3|DESC_D|SEGM_TYPE_TSS);

    load_gdt((void*) ((_size_t) &gdt_list + PAGE_OFFSET), GDT_LENGTH);
}
